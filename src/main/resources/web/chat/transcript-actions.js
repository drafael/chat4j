(function () {
  ...
  function dispatchTranscriptAction(action, messageIndex, text) {
    ...
    if (
      ...
      window.chat4jTranscriptAction(
        JSON.stringify({
          args: [normalizedAction, normalizedMessageIndex, payloadText],
        }),
      );
    }
  }
  ...
  function showSourcePreview(anchor, event) {
    ...
    preview.querySelector(".source-preview-favicon").textContent =
      sourceInitial(anchor.getAttribute("data-source-domain"));
    preview.querySelector(".source-preview-domain").textContent =
      anchor.getAttribute("data-source-domain") || "";
    preview.querySelector(".source-preview-title").textContent =
      anchor.getAttribute("data-source-title") || anchor.textContent || "";
    preview.querySelector(".source-preview-snippet").textContent =
      anchor.getAttribute("data-source-snippet") || "";
    preview.classList.add("visible");
    preview.setAttribute("aria-hidden", "false");
    positionSourcePreview(
      event || {
  ...
  function dispatchMessageActionButton(button, event) {
    ...
    var text =
      action === "read-aloud"
        ? button.getAttribute("data-read-aloud-token") || ""
        : "";
  ...
  function showTranscriptMenu(event, row) {
    ...
    var readAloudActive =
      hasReadAloud &&
      readAloudButton.getAttribute("data-read-aloud-active") === "true";
    var regenerateLabel = row.classList.contains("user")
      ? "Regenerate Response"
    ...
    var readAloud = hasReadAloud
      ? '<div class="transcript-menu-separator"></div><button data-action="read-aloud"><span class="icon ' +
        (readAloudActive ? "player-stop" : "read-aloud") +
        '" aria-hidden="true"></span><span class="label">' +
        (readAloudActive ? "Stop" : "Read aloud") +
        '</span><span class="shortcut"></span></button>'
      : "";
    var exportDisabled =
      window.chat4jPdfExportAvailable === true ? "" : " disabled";
    ...
    menu.innerHTML =
      ...
      '<button data-action="regenerate"><span class="icon regenerate" aria-hidden="true"></span><span class="label">' +
      regenerateLabel +
      '</span><span class="shortcut"></span></button>' +
      '<button data-action="export-pdf"' +
      exportDisabled +
      '><span class="icon open-diagram" aria-hidden="true"></span><span class="label">Export to PDF…</span><span class="shortcut"></span></button>';
    menu._chat4jDiagram = diagram || null;
    Array.prototype.forEach.call(
      menu.querySelectorAll("button[data-action]"),
      function (button) {
        button.onclick = function (clickEvent) {
          ...
          var text =
            action === "copy-selected"
              ...
              : action === "read-aloud"
                ? readAloudButton.getAttribute("data-read-aloud-token") || ""
                : "";
    ...
    menu.style.left =
      Math.min(event.clientX, Math.max(0, window.innerWidth - 240)) + "px";
    menu.style.top =
      Math.min(event.clientY, Math.max(0, window.innerHeight - 120)) + "px";
    menu.style.display = "block";
  }
  function installCodeCopyButtons() {
    Array.prototype.forEach.call(
      document.querySelectorAll("table.md-code-block"),
      function (table) {
        if (
          table.parentNode &&
          table.parentNode.classList &&
          table.parentNode.classList.contains("code-block-shell")
        ) {
          return;
        }
        ...
        button.onclick = function (event) {
          var code =
            table.querySelector("tr.code-body pre") ||
            table.querySelector("pre") ||
            table;
          dispatchTranscriptAction(
            "copy-text",
            -1,
  ...
  function installCustomScrollbar() {
    ...
    document.addEventListener(
      ...
      function (event) {
        ...
        root.scrollTop =
          dragStartScrollTop +
          ((event.clientY - dragStartY) / trackHeight) * scrollRange;
        updateCustomScrollbar();
        event.preventDefault();
      },
      true,
  ...
  window.chat4jInstallTranscriptActions = function () {
    ...
    if (window.chat4jRenderEnhancements) {
      window.chat4jRenderEnhancements(
        document.querySelector(".transcript") || document.body,
      );
    }
    ...
    function (event) {
      ...
      if (attachmentButton) {
        ...
        dispatchTranscriptAction(
          "open-attachment",
          -1,
          attachmentButton.getAttribute("data-attachment-path") || "",
        );
        return;
      }
      var activityCopyButton = closest(
        event.target,
        'button[data-action="copy-activity"]',
      );
      if (activityCopyButton) {
        var activityBox = closest(activityCopyButton, ".activity-box");
        var activityContent = activityBox
          ? activityBox.querySelector(".activity-content")
          : null;
        var activitySummary = activityBox
          ? activityBox.querySelector("summary")
          : null;
        var text =
          activityContent &&
          String(activityContent.textContent || "").trim().length > 0
      ...
      var actionButton = closest(
        event.target,
        "button[data-action][data-message-index]",
      );
      if (actionButton) {
