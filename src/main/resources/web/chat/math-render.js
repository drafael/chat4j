(function () {
  ...
  function renderIntoReplacement(sourceNode, parsed) {
    ...
    target.className = parsed.displayMode
      ? "chat4j-math-display"
      : "chat4j-math-inline";
  ...
  function renderInlineMath(root) {
    Array.prototype.forEach.call(
      root.querySelectorAll(
        "code.md-latex-inline:not([data-chat4j-math-rendered])",
      ),
      function (code) {
        try {
          renderIntoReplacement(
            code,
            parseDelimitedMath(code.textContent, false),
          );
        } catch (error) {
  ...
  function renderDisplayMath(root) {
    Array.prototype.forEach.call(
      root.querySelectorAll(
        "table.md-latex-block:not([data-chat4j-math-rendered])",
      ),
      function (table) {
        var pre =
        ...
        try {
          renderIntoReplacement(
            closestCodeBlockShell(table) || table,
            parseDelimitedMath(pre.textContent, true),
          );
        } catch (error) {
