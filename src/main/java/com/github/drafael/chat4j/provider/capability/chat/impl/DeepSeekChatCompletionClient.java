...
public class DeepSeekChatCompletionClient implements ChatCompletionClient {
    ...
    public DeepSeekChatCompletionClient(@NonNull ProviderAttachmentSupport attachmentSupport) {
        this(
                new OpenAiChatCompletionClient(attachmentSupport),
                new DeepSeekAnthropicWebSearchClient(attachmentSupport)
        );
    }

    DeepSeekChatCompletionClient(ChatCompletionClient ordinaryClient, ChatCompletionClient nativeSearchClient) {
    ...
    public void streamCompletion(
        ...
        if (!DeepSeekNativeWebSearchSupport.supports(
                runtime.descriptor().name(),
                runtime.selectedModel(),
                runtime.baseUrl()
        )) {
            throw new IllegalArgumentException(
                    "DeepSeek native Web Search requires a supported V4 model and the official API endpoint."
            );
        }
