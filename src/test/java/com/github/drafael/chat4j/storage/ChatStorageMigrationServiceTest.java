...
class ChatStorageMigrationServiceTest {
    ...
    void setUp() throws Exception {
        ...
        cleanStorage();
        settingsRepo = new SettingsRepo(storagePaths);
    }
    ...
    void conversationRepo_whenUsingSqlite_persistsAndReadsConversation() throws Exception {
        DataSource sqliteDataSource = ChatDataSourceFactory.create(storagePaths, StorageBackend.SQLITE);
        new DatabaseBootstrap(storagePaths, sqliteDataSource, SqlDialects.forBackend(StorageBackend.SQLITE)).init();
        var subject = new ConversationRepo(
                sqliteDataSource,
    ...
    void migrateIfNeeded_whenPendingIsH2_copiesSqliteDataAndMarksH2Active() throws Exception {
        ...
        var subject = new ChatStorageMigrationService(storagePaths, settingsRepo);
        ...
        DataSource h2DataSource = ChatDataSourceFactory.create(storagePaths, StorageBackend.H2);
        var h2Repo = new ConversationRepo(
                h2DataSource,
                storagePaths.attachmentsDirectory(),
    ...
    void migrateIfNeeded_whenPendingIsSqlite_copiesH2DataAndMarksSqliteActive() throws Exception {
        ...
        DataSource sqliteDataSource = ChatDataSourceFactory.create(storagePaths, StorageBackend.SQLITE);
        var sqliteRepo = new ConversationRepo(
                sqliteDataSource,
                storagePaths.attachmentsDirectory(),
                SqlDialects.forBackend(StorageBackend.SQLITE)
        );
