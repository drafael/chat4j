...
2. Chat4J starts a local callback listener when possible.
3. Chat4J shows a modal dialog with the login URL, **Open browser**, **Copy login URL**, and a callback URL input.
4. Chat4J attempts to open the browser automatically after the dialog is visible.
5. If browser callback succeeds, the dialog closes and login completes automatically.
6. If localhost callback is blocked, user copies the full browser address bar URL and pastes it into the dialog. Expected format: `http://localhost:1455/auth/callback?code=...&state=...`.
7. Chat4J exchanges authorization code for token(s), then resolves a usable bearer token.
8. Chat4J stores token metadata in `~/.config/chat4j/codex-auth.json`.
...
- `ProvidersPanel` shows simplified Codex auth status (`Authorized` / `Not authorized`) and login/logout actions.
