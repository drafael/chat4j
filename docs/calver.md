...
GitHub Releases use native autogenerated release notes. The release workflow keeps `generate_release_notes: true`, and `.github/release.yml` groups merged pull requests into user-facing categories.

Use one changelog label on every human PR when possible:

- `feature` or `enhancement` for user-visible additions.
- `bug` or `fix` for user-visible fixes.
- `security` for vulnerability fixes or hardening.
- `documentation` or `docs` for docs-only changes.
- `maintenance`, `chore`, `ci`, `build`, `refactor`, or `test` for internal upkeep.

Dependency update PRs should keep the dependency labels provided by Dependabot. Use `skip-changelog` only for changes that should not appear in release notes. Write PR titles as release-note fragments, for example `Fix Anthropic reasoning smoke model selection` rather than `Update stuff`.
